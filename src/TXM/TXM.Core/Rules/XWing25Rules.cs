...
namespace TXM.Core
{
    ...
    public class XWing25Rules : AbstractRules
    {
        ...
        {
            List<Player> t = unsorted.OrderByDescending(x => x.IsInCut).ThenByDescending(x => x.TournamentPoints).ThenByDescending(x => x.StrengthOfSchedule).ThenByDescending(x => x.MarginOfVictory).ThenByDescending(x => x.Order).ToList<Player>();
