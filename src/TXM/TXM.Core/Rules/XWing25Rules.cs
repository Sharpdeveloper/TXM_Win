...
namespace TXM.Core
{
    ...
    public class XWing25Rules : AbstractRules
    {
        ...
        {
            ObservableCollection<Models.Player> t = (ObservableCollection<Models.Player>)unsorted.OrderByDescending(x => x.TournamentPoints).ThenByDescending(x => x.StrengthOfSchedule).ThenBy(x => x.MarginOfVictory).ThenBy(x => x.Order);
