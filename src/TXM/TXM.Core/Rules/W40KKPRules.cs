...
namespace TXM.Core
{
	...
	public class W40KKPRules : AbstractRules
	{
        ...
        public override List<Player> SortTable(List<Player> unsorted)
        {
            ...
            List<Player> t = unsorted.OrderByDescending(x => x.IsInCut).ThenByDescending(x => x.TournamentPoints).ThenByDescending(x => x.MarginOfVictory).ThenByDescending(x => x.Order).ToList<Player>();
