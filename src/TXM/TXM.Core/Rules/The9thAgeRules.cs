...
namespace TXM.Core
{
	...
	public class The9thAgeRules : AbstractRules
	{
        ...
        {
            ObservableCollection<Models.Player> t = (ObservableCollection<Models.Player>)unsorted.OrderByDescending(x => x.TournamentPoints).ThenByDescending(x => x.Wins).ThenBy(x => x.Order);
